...
export default function TradeDetailsModal({ trade, open, onOpenChange }: TradeDetailsModalProps) {
  ...
  const profitPercentage = calculatePnLPercentage(trade, userStats?.accountSize || 0);
      ...
      <DialogContent className="max-w-2xl w-[95vw] max-h-[90vh] overflow-y-auto bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 border border-purple-700">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-purple-300">Symbol</p>
          ...
          <div className="space-y-3">
            <h4 className="font-semibold text-white text-lg">Position Details</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <p className="text-purple-300">Lot Size</p>
                <p className="text-white font-mono text-base">{trade.lots || "0"}</p>
              </div>
              <div className="space-y-1">
                <p className="text-purple-300">Volume</p>
                <p className="text-white font-mono text-base">
                  {trade.volume || (trade.lots ? (trade.lots * 100000).toLocaleString() : "0")}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-purple-300">Open Price</p>
                <p className="text-white font-mono text-base">
                  {trade.openPrice ? trade.openPrice.toFixed(5) : "0.00000"}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-purple-300">Close Price</p>
                <p className="text-white font-mono text-base">
                  {trade.closePrice ? trade.closePrice.toFixed(5) : "Still Open"}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Risk Management */}
          <div className="space-y-3">
            <h4 className="font-semibold text-white text-lg">Risk Management</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <p className="text-purple-300">Stop Loss</p>
                <p className="text-white font-mono text-base">
                  {trade.stopLoss ? trade.stopLoss.toFixed(5) : "None"}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-purple-300">Take Profit</p>
                <p className="text-white font-mono text-base">
          ...
          {/* Timing */}
          <div className="space-y-3">
            <h4 className="font-semibold text-white text-lg">Timing</h4>
            <div className="space-y-3 text-sm">
              <div className="space-y-1">
                <p className="text-purple-300">Open Time</p>
                <p className="text-white">{trade.openTime ? formatDateTime(trade.openTime) : "-"}</p>
              </div>
              {trade.closeTime && (
                <div className="space-y-1">
                  <p className="text-purple-300">Close Time</p>
                  <p className="text-white">{formatDateTime(trade.closeTime)}</p>
                </div>
              )}
              <div className="space-y-1">
          ...
          {/* P&L Details */}
          <div className="space-y-3">
            <h4 className="font-semibold text-white text-lg">Profit & Loss</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <p className="text-purple-300">P&L</p>
                <p 
              ...
              <div className="space-y-1">
                ...
                >
                  {isNaN(profitPercentage) || !isFinite(profitPercentage) ? "0.00%" : 
                    `${profitPercentage >= 0 ? "+" : ""}${profitPercentage.toFixed(2)}%`}
                </p>
                {userStats?.accountSize && (
                  <p className="text-xs text-purple-400">
                    of ${formatCurrency(userStats.accountSize).replace('$', '')} account
                  </p>
                )}
              </div>
              <div className="space-y-1">
