...
interface CheckoutFormProps {
  ...
  refreshPaymentIntent: (amount: number, promo?: string) => void;
  clientSecret: string;
}
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      if (data.valid) {
        ...
        // Refresh payment intent with new price
        refreshPaymentIntent(data.finalPrice, promoCode);
...
export default function Checkout() {
  ...
  const refreshPaymentIntent = async (amount: number, promo?: string) => {
    try {
      setIsLoading(true);
      ...
      if (data.freeAccess) {
        ...
        setClientSecret(""); // No need for clientSecret in free access
        toast({
  ...
  // Handle free access case without Stripe Elements
  if (isFreeAccess) {
    return (
      <CheckoutForm 
